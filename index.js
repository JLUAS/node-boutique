...
app.post('/register/admin', async (req, res) => {
  const { username, email, password, nombre_negocio, ubicacion, contacto } = req.body;
  const role = 'admin';
  ...
  pool.getConnection((err, connection) => {
    ...
    connection.beginTransaction(err => {
      ...
      connection.query('INSERT INTO usuarios (nombre, email, password, rol, nombre_negocio, ubicacion, contacto) VALUES (?, ?, ?, ?, ?, ?, ?)', [username, email, hashedPassword, 'admin', nombre_negocio, ubicacion, contacto], (err, result) => {
        if (err) {
          connection.rollback(() => {
            connection.release();
            return res.status(500).send(err);
